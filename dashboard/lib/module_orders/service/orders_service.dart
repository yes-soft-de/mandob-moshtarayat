...
import 'package:mandob_moshtarayat_dashboad/module_orders/manager/my_orders_manager.dart';
...
class OrdersService {
  ...
  Future<DataModel> getPendingOrders() async {
    MyOrdersResponse? _ordersResponse =
        await _myOrdersManager.getPendingOrders();
  ...
  Future<DataModel> getOngoingOrders() async {
    MyOrdersResponse? _ordersResponse =
        await _myOrdersManager.getOngoingOrders();
    if (_ordersResponse == null) {
      return DataModel.withError(S.current.networkError);
    }
    if (_ordersResponse.statusCode != '200') {
  ...
  Future<DataModel> getOrdersWithoutPending() async {
    MyOrdersResponse? _ordersResponse =
        await _myOrdersManager.getOrdersWithoutPending();
    if (_ordersResponse == null) {
      return DataModel.withError(S.current.networkError);
    }
    if (_ordersResponse.statusCode != '200') {
  ...
  Future<DataModel> getFilteredDateOrders(
      String firstDate, String lastDate) async {
    MyOrdersResponse? _ordersResponse =
        await _myOrdersManager.getFilteredDateOrders(firstDate, lastDate);
    if (_ordersResponse == null) {
      return DataModel.withError(S.current.networkError);
    }
    if (_ordersResponse.statusCode != '200') {
