...
class RoomIdHelperService
{
    ...
    public function roomIdGenerate()
    {
        do
        {
            // keep generating roomID while it is exist
            $found = false;
            ...
            $roomID = vsprintf('%s%s-%s-%s-%s-%s%s%s', str_split(bin2hex($data), 4));

            // check if roomID exist
            $result = $this->roomIdHelperManager->getStoreOwnerProfileByRoomID($roomID);
