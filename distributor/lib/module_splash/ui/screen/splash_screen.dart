...
class _SplashScreenState extends State<SplashScreen> {
  ...
  _getNextRoutFirst(){
    if (widget._authService.isLoggedIn) {
      return   Navigator.of(context).pushNamedAndRemoveUntil(HomeRouts.HOME_SCREEN, (route) => false);
    }
    return Navigator.of(context).pushNamedAndRemoveUntil(AuthorizationRoutes.LOGIN_SCREEN, (route) => false);
  }

  Future<String> _getNextRoute() async {
    await Future.delayed(Duration(seconds: 3));
    if (widget._authService.isLoggedIn) {
      return HomeRouts.HOME_SCREEN;
    }
